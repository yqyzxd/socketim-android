...
import com.wind.im.ui.MsgAdapter;
import com.wind.im.ui.widget.HeadImageView;
import com.wind.im.util.TimeUtil;
 ...
 */
public abstract class MsgViewHolderBase extends RecyclerViewHolder<BaseMultiItemFetchLoadAdapter, BaseViewHolder, IMessage> {
    ...
    protected int leftBackground() {
        return IMContext.getOptions().messageLeftBackground;
    }

    // 当是发送出去的消息时，内容区域背景的drawable id
    protected int rightBackground() {
        return IMContext.getOptions().messageRightBackground;
    }
    ...
    protected boolean shouldDisplayNick() {
        return message.getSessionType() == SessionType.TEAM && isReceivedMessage() && !isMiddleItem();
    }
    ...
    protected boolean isReceivedMessage() {
        return message.getDirect() == MsgDirection.In;
    }

    /// -- 以下是基类实现代码
    @Override
    public void convert(BaseViewHolder holder, IMessage data, int position, boolean isScrolling) {
        view = holder.getConvertView();
        context = holder.getContext();
        message = data;
       // System.out.println("MsgItem convert itemCount"+getMsgAdapter().getItemCount());
        inflate();
        refresh();
        bindHolder(holder);
    }
    ...
    protected final void inflate() {
        ...
        IMSession session=getMsgAdapter().getContainer().session;
        avatarLeft.setSession(session);
        avatarRight.setSession(session);
    ...
    private void setTimeTextView() {
        ...
        String text = TimeUtil.getTimeShowString(message.getTimestamp(), false);
    ...
    private void setReadReceipt() {
        ...
        if (SessionType.P2P== message.getSessionType()) {
            if (isReceivedMessage() ||!shouldDisplayReceipt()){
                readReceiptTextView.setVisibility(View.GONE);
                return;
            }
            if (shouldDisplayReceipt() && receiveReceiptCheck(message)){
            ...
            //todo 目前全部为已读
            if (shouldDisplayReceipt()&& !isReceivedMessage()/* && !TextUtils.isEmpty(getMsgAdapter().getUuid()) && message.getUuid().equals(getMsgAdapter().getUuid())*/) {
                readReceiptTextView.setText("已读");
                readReceiptTextView.setVisibility(View.VISIBLE);
            } else {
                readReceiptTextView.setVisibility(View.GONE);
    ...
    private boolean receiveReceiptCheck(final IMessage msg) {
        return msg != null
            ...
            /*    && msg.getMsgType() != MsgType.tip
                && msg.getMsgType() != MsgType.notification*/
                && msg.isRemoteRead();

    }

    private void setAckMsg() {
        if (message.getSessionType() == SessionType.TEAM && message.needMsgAck()) {
            if (isReceivedMessage()) {
                // 收到的需要已读回执的消息，需要给个反馈
